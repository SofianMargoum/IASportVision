...
try {
  ...
  // Route pour récupérer les données de la table 'effectif' avec une condition sur 'nom'
  router.get('/effectif', async (req, res) => {
    if (!pool) {
    ...
    try {
      ...
      const { nom } = req.query;
      let query = 'SELECT * FROM effectif';
      const values = [];
  ...
  router.use((err, req, res, next) => {
    console.error('Erreur générale dans le service effectif :', err.message);
    console.error(err.stack);
    res.status(500).json({
      message: 'Erreur générale dans le service effectif.',
      error: err.message,
    });
  });
  ...
  router.get('/effectif', (req, res) => {
    res.status(200).json({ message: 'effectif error last' });
  });
}
