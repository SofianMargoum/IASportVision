...
async function batchMergeChunksFromGcs({ rollingId, meta, chunks }) {
  ...
  try {
    ...
    // 1) Download current merged in parallel with all chunks
    const downloadTasks = [];
    ...
    // Assemble file list in order: merged_prev (if any), then chunks sorted by index
    const mergedResult = dlResults.find(r => r.type === 'merged');
    if (mergedResult) localFiles.push(mergedResult.localPath);

    const chunkResults = dlResults.filter(r => r.type === 'chunk').sort((a, b) => a.index - b.index);
    for (const cr of chunkResults) localFiles.push(cr.localPath);
    ...
    if (localFiles.length === 1) {
      fs.copyFileSync(localFiles[0], outLocal);
      mergeMode = 'batch_copy_single';
    } else {
      const mergeInfo = await concatMp4Files(localFiles, outLocal, { timeoutMs: 5 * 60 * 1000 });
      mergeMode = `batch_${mergeInfo?.mode || 'concat'}`;
    }
